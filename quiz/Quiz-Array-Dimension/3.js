...
function minDistanceBetweenGreatest(arr) {
  ...
  arr.forEach((element, index) => {
    if (element === max) {
      maxNumsIndex.push(index);
    }
  });

  if (maxNumsIndex.length === 1) return 0;

  let min = Math.abs(maxNumsIndex[0] - maxNumsIndex[1]);

  for (let i = 0; i < maxNumsIndex.length - 1; i++) {
    const currentDistance = Math.abs(maxNumsIndex[i] - maxNumsIndex[i + 1]);
    if (min > currentDistance) {
      min = Math.abs(currentDistance);
    }
  }
  return min;
}
